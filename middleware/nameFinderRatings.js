...
module.exports = (req,res) => {
     ...
     let food_price = null;
        ...
        for (let i=0; i<menu.length; i++){
        	for(let j=0; j<menu[i].items.length; j++){
        		if(req.body.food_id === menu[i].items[j].apiKey){
                    ...
                    food_price = menu[i].items[j].basePrice;
        		}
        	}
        }
        ...
        let user_data = {
            ...
            user: d,
            ratings_value: req.body.ratings_value
        }
    Rating.getRatingById(user_data, (err, data) => {
        if(err){
            throw err; 
        }else if (!(data.length != 0)){
        Rating.addRating(user_data, (err, data) => {
            if(err) {
                throw err; 
            }else if (data.length != 0){
                    res.status(201).json({
                        success: true,
                        data: data,
                        message: 'Created. Ratings Stored Succesfully'
                    }) 
                } else if (!data) {
                res.status(404).json({
                    success: false,
        ...
        res.status(400).json({
                    success: false,
                    message: 'Bad Request. Ratings Already stored for this user, restaurant and food.'
                });
    }else{
        res.status(400).json({
                    success: false,
